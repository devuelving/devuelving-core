...
class Product extends Model
{
    ...
    {
        $return = [];
        $productProviders = ProductProvider::where('product', $this->id)->get();
    ...
    public function getProductProvider($cheapest = false)
    {
        try {
            if ($this->franchise === null) {
                ...
                $productProvider = ProductProvider::join('provider', 'product_provider.provider', '=', 'provider.id')
                ...
                $productProvider = ProductProvider::where('product', $this->id)->orderBy('product_provider.cost_price', $rule)->first();
            }
    ...
    public function getPublicPriceCost()
    {
        if ($this->franchise === null) {
            if ($this->getProductProviderData('cost_price') != null) {
                ...
                $provider = Provider::find($this->getProductProviderData('provider'));
    ...
    {
        $productCustom = ProductCustom::where('product', $this->id)->where('franchise', \App\Franchise::get('code'))->whereNotNull('price')->get();
    ...
    {
        try {
            $productCustom = ProductCustom::where('product', $this->id)->where('franchise', \App\Franchise::get('code'))->whereNotNull('price')->first();
    ...
    {
        $productCustom = ProductCustom::where('product', $this->id)->where('franchise', \App\Franchise::get('code'))->whereNotNull('promotion')->get();
        if (count($productCustom) == 0) {
            return false;
        } else {
            return true;
    ...
    public function getPrice()
    {
        if ($this->getProductProviderData('default_price') != null) {
            if ($this->checkCustomPrice()) {
                $productCustom = ProductCustom::where('product', $this->id)->where('franchise', \App\Franchise::get('code'))->first();
                if ($productCustom->price_type == 1) {
    ...
    {
        $productCustom = ProductCustom::get(\App\Franchise::get('code'), $this->id);
        if ($options['action'] == 'price') {
            ...
            $productCustom->save();
            ProductCustom::checkClear($productCustom->id);
            return [
                'status' => true,
    ...
    public function getStock()
    {
        if (! $this->unavailable) {
            if ($this->stock_type == 1) {
                $additions = ProductStock::where('product_stock.type', '=', 2)
                ->where('product_stock.product', '=', $this->id)
                ->sum('stock');
                $subtractions = ProductStock::where('product_stock.type', '=', 1)
                ->where('product_stock.product', '=', $this->id)
                ->sum('stock');
